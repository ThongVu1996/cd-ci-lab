...
2.  Tạo file \`.netdev\` (Tạo interface ảo `macvlan-host`):
...
Cấu hình Docker Daemon trên Host OS để tin tưởng Registry nội bộ và biết sử dụng Technitium DNS (`192.168.1.30`) để phân giải tên miền.

**Áp dụng cho cả IPVLAN và MACVLAN.**
...
- #### Bước 2.4: Cấu hình DNS Server (Host OS) - CẬP NHẬT QUAN TRỌNG

  Đảm bảo Host OS dùng Technitium DNS (`192.168.1.30`) để phân giải các tên miền cục bộ.

  **⚠️ Thay đổi quan trọng:** Bạn phải **XÓA HẲN** file \`resolv.conf\` cũ đi rồi mới tạo lại. Việc sửa trực tiếp (overwrite) đôi khi không hoạt động do file này là symlink.
...
Thực hiện bước này sau khi đã hoàn thành các cấu hình Docker Daemon và DNS Host ở Bước 2.3 và 2.4.
...
2. Nginx Proxy Manager (`http://192.168.1.31:81`):
   ...
   **Chi tiết: Cách thêm Proxy Host (URL) trong NPM**
   1. Đăng nhập vào NPM: `http://192.168.1.31:81` (Email mặc định: `admin@example.com`, Pass: `changeme`).
   2. Vào "Hosts" -&gt; "Proxy Hosts" -&gt; "Add Proxy Host".
   3. **Tab Details:**
   ...
   4. **Tab SSL:** (Xem bước SSL bên dưới).
   5. Nhấn "Save".
   6. Lặp lại cho các host còn lại.
     ...
     **Gặp lỗi `Exec format error`?**  
     ...
     - **Cài đặt CA trên máy thật:**
       ...
       - **Trên Windows:**
         ...
         6. Nhấn "Next" -&gt; "Finish".
     5. **Hoàn tất:** Khởi động lại trình duyệt của bạn (Chrome, Firefox...). Bây giờ khi truy cập `https://gitlab.thongdev.site`, bạn sẽ thấy ổ khóa màu xanh!

     6. **💡 Mẹo Fix Bug: Trình duyệt vẫn báo "Not Secure" sau khi đã Trust?**
...
1. Mở Chrome, gõ vào thanh địa chỉ: `chrome://net-internals/#dns` -&gt; Nhấn **Clear host cache**.
2. Gõ tiếp: `chrome://net-internals/#sockets` -&gt; Nhấn **Flush socket pools** (Nút này quan trọng nhất).
3. Khởi động lại Chrome và thử truy cập lại.
